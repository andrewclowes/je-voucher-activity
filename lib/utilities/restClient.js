  ...
  RestClient.prototype[method] = function (name, options, callback) {
    ...
    this.restClient[method](options, function(err, response, body) {
      ...
      if (!err && self.statsClient) {
        self.statsClient.increment("dependencies." + self.apiName + "." + name + "." + method.toUpperCase() + "." + response.statusCode);
        self.statsClient.timing("dependencies." + self.apiName + "." + name + "." + method.toUpperCase() + "." + response.statusCode, duration);
      }
